...
export class HostLogicOpsClient {
    ...
    private readonly monitoringToken: string;
    private readonly actingUserId: string;

    constructor(monitoringToken: string, actingUserId: string, baseUrl?: string) {
        if (!monitoringToken) throw new Error('MONITORING_MOBILE_API_TOKEN is required');
        if (!/^\d+$/.test(actingUserId)) throw new Error('HOSTLOGIC_OPS_ACTING_USER_ID must be a positive integer');
